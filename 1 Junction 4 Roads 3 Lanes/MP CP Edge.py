...
def run(simulation_duration):
    ...
    edges = traci.edge.getIDList() #get edge ids
    ...
    while traci.simulation.getTime() < simulation_duration: #run simulation till simulation time greater than equal to simulation duration
            
            # get the current simulation time
            current_time = traci.simulation.getTime()

            #write the waiting time entries over time 
            total_waiting_time = 0 #waiting time in the network at that time
            for lane in lanes:
            ...
            if current_time % phase_duration == 0:
                max_waiting_cars_edge = 0
                max_waiting_edge = None
                for edge in edges:
                    waiting_cars_edge = traci.edge.getLastStepVehicleNumber(edge) # number of cars in the edge
                    #get max number of waitng cars and its corresponding edge
                    if waiting_cars_edge > max_waiting_cars_edge:
                        max_waiting_cars_edge = waiting_cars_edge
                        max_waiting_edge = edge
                if max_waiting_edge == "-E0":
                    traci.trafficlight.setRedYellowGreenState(junction_id, neg_E0 )
                
                if max_waiting_edge == "-E1":
                    traci.trafficlight.setRedYellowGreenState(junction_id, neg_E1 )
               
                if max_waiting_edge == "-E2":
                    traci.trafficlight.setRedYellowGreenState(junction_id, neg_E2 )
                
                if max_waiting_edge == "-E3":
...
# save the plot
plt.savefig('Waiting Time Vs Simulation MP CP Edge.png')
